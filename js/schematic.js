...
function updateSchematicReadouts(){
  ...
  function bubbleFor(it, i, count, flip){
    ...
    if(hasValve){
      ...
      else if(it.id==='hotdischarge'){ lines=['HD-SAT '+fmt(sim.hotDeckTemp,1)+'\u00b0F', 'HD-RH '+fmt(rhFromW(sim.hotDeckTemp, sim.W_supply)*100,0)+'%', fmt(sim.hotDeckCfm,0)+' CFM', 'SP '+fmt(sim.spDeckHot || 0, 2)+'"']; }
      if(lines){
